...
func main() {
	...
	if agentConfig.EnableRelayModeForXds {
		err := bootstrap.CreateRelayBootstrapYamlFile(agentConfig, false)
		if err != nil {
		...
		if agentConfig.EnableLocalRelayModeForXds {
			...
			if err = bootstrap.CreateRelayBootstrapYamlFile(agentConfig, true); err != nil {
	...
	// Start the configured local relay Envoy binary and keep it alive
	if agentConfig.EnableLocalRelayModeForXds {
